...
class FeatureGenerationRecommender(IngredientRecommender):
    def __init__(self,
        ...
        self.labelled_cols = [
            "Savoury","Rough","Hot","Spicy","Acidic","Crunchy","Creamy","Sticky","Liquid","Aromatic","Salty","Citrusy","Herbal","Fluffy","Flaky","Cooling","Chunky","Fishy","Firm"
            ]
    
    def train_classifiers(self):
        
        # import labelled dataset
        labelled_df = pd.read_csv("./feature_generation/trimmed_labelled_slim.csv",nrows=300)

        # remove double quotes from ingredient string
        labelled_df["RecipeIngredientParts"] = labelled_df["RecipeIngredientParts"].str.replace("\"","")
        
        model_name = "paraphrase-MiniLM-L6-v2"
        
        # encode ingredient string
        self.bert_encoder = SentenceTransformer(model_name)
        encoded_ingredients = self.bert_encoder.encode(labelled_df["RecipeIngredientParts"].values)
    ...
    def generate_features(self, verbose=True):
        ...
        if verbose: print(f"Loaded {full_dataset.shape} recipes...")
        ...
        if verbose: print("Encoding ingredients to SBERT vectors:")
        encoded_ingredients = self.bert_encoder.encode(full_dataset["RecipeIngredientParts"].values)
        if verbose: print("Predicting labels for food attributes")
        for col in tqdm(self.labelled_cols):
            
            full_dataset[col] = self.classifiers[col].predict(encoded_ingredients)
            
        if verbose: print("Writing to CSV")
